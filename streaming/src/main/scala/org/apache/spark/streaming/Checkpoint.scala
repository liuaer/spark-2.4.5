...
private[streaming]
/*
       检查点是很多分布式系统为了容灾容错引入的机制，其实质是将系统运行期的内存数
    据结构和状态持久化到磁盘上，在需要时通过对这些持久化数据的读取，重新构造出之前
    的运行期状态。Spark使用检查点主要是为了将RDD的执行状态保留下来， 在重新执行
    时就不用重新计算， 而直接从检查点读取。Checkpoint RDD是对检查点数据进行操作的
    RDD， 例如， 读写检查点数据。RDD Checkpoint Data表示RDD检查点的数据。Spark的检
    查点离不开Checkpoint RDD和RDD Check po it Data的支持， 本节将对它们的代码实现进行
    分析。
