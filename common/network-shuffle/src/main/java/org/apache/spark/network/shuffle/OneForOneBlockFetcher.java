...
public class OneForOneBlockFetcher {
  private static final Logger logger = LoggerFactory.getLogger(OneForOneBlockFetcher.class);
   /**用于向服务端发送请求的 TransportClient*/
  private final TransportClient client;
    /**
     * 即 Open Blocks , OpenBlocks将携带远端节点的appid(应用程序标识)
     * execId( Executor标识)和 blockIds( BlockId的数组),这表示从远端的哪个实例获
     * 取哪些 Block,并且知道是哪个远端 Executor生成的Block
     */
  ...
  private final DownloadFileManager downloadFileManager;
    /**
     * 根据代码清单6-103,客户端给服务端发送 OpenBlocks消息后,服
     * 务端会在 OneForOneStreamManager的 streams缓存中缓存从存储体系中读取到
     * 的 ManagedBuffer序列,并生成与 ManagedBuffer序列对应的 streamId,然后将
     * streamId和 ManagedBuffer序列的大小封装为 StreamHandle消息返回给客户端,客
     * 户端的 streamHandle属性将持有此Streamhandle消息。
     */
