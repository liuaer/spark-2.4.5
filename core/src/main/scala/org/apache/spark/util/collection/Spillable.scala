...
private[spark] abstract class Spillable[C](taskMemoryManager: TaskMemoryManager)
  ...
  // For testing only
  /**
    * 对集合的内存使用进行跟踪的初始内存阈值。可通过
    * spark.shuffle.spill.initialMemoryThreshold 属性配置， 默认为 5MB。
    */
  ...
  /**
    ...
    * 可通过 spark.shuffle.spill.numElementsForceSpillThreshold 属性配置， 默认为Long.MAX_VALUE
    */
  ...
  /**
    * 对集合的内存使用进行跟踪的初始内存阈值。myMemoryThreshold的初始值等于 initialMemoryThreshold
    */
  ...
  /**
   *  maybeSpil方法用于将 PartitonedAppendOnlyMap或 PartionedPairBuffer底层的数据
    * 溢出到磁盘
    *
    * @param collection collection to spill to disk
  ...
  protected def maybeSpill(collection: C, currentMemory: Long): Boolean = {
    ...
    if (elementsRead % 32 == 0 && currentMemory >= myMemoryThreshold) {
      ...
      // 申请2 倍的预估内存大小，若还是不够用，则准备溢出
      // If we were granted too little memory to grow further (either tryToAcquire returned 0,
      // or we already had more memory than myMemoryThreshold), spill the current collection
      shouldSpill = currentMemory >= myMemoryThreshold
    }
    // 前面申请的内存不够用 或 当前的元素过多，则应当溢出
