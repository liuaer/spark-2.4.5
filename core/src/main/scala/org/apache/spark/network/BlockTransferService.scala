...
/**
  * 两个子类 MockBlockTransferService 、 NettyBlockTransferService
  * MockBlockTransferService   用于测试
  * NettyBlockTransferService  传输服务靠这个
  *
  * Spark是分布式部署的,每个Task(准确说是任务尝试)最终都运行在不同的机器节点上
  * map任务的输出结果直接存储到map任务所在机器的存储体系中 <所以只有在有shuffle任务时才会使用到BlockTransferService>
