...
abstract class RDD[T: ClassTag](
    ...
    @transient private var deps: Seq[Dependency[_]] // 存储当前的依赖
  ...
  /**
    *
    * 锁定此RDD的所有可变状态(持久性、分区、依赖项等)。我们不要使用' this '，
    * 因为RDDs是用户可见的，所以用户可能已经添加了自己的锁定在RDDs;共享可能会导致僵局。
    * <br>
    * 对于一个RDD依赖关系链，一个线程可能持有其中许多的锁;但因为DAG是无环的，
    * 而且我们只对DAG中的一个路径持有锁，所以没有死锁的机会。
    *
    * Integer的使用很简单，所以它是可序列化的——executor可以引用共享的
    *
    * <br> <br>
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    /*
       storageLevel != StorageLevel.NONE ---> 至少会使用磁盘、堆内内存、堆外内存三者之一，
       因此可以调用getOrCompute方法从这些存储中尝试获取计算结果。

         如果RDD的存储级别(Storage Level) 是NONE， 那么说明分区任务可能是初次执行且
         存储中还没有任务的执行结果， 所以会调用 computeOrReadCheckpoint 方法计算或者从检查点恢复
     */
    if (storageLevel != StorageLevel.NONE) {
      // 从缓存中读取或者重新计算对应Partition的数据，注意！！！，重新计算调用了 computeOrReadCheckpoint
      /*
         老版本 2.0 之前是，现在直接放到RDD 方法内
  ...
  {
    /*
        假设本RDD为 ReliableCheckpointRDD
        如果checkpointData中保存了 ReliableCheckpointRDD 且其检查点的状态(cpState)是
        Checkpointed， 那么调用firstParent方法找到其父RDD， 然后调用父RDD的iterator方法。
        由于firstParent中调用了dependencies， 且当前RDD的父RDD实际是 ReliableCheckpointRDD，
        那么对ReliableCheckpointRDD 的 iterator方法的调用最终将转变为对 ReliableCheckpointRDD
        的compute方法的调用， 从而从检查点文件读取之前保存的计算结果。
        ...
        疑问？ 为什么是  firstParent，而不是 dependencies
        解答，先一直找父RDD，到头之后，然后依次计算与 split分区相关的数据，因为我们可能存在 shuffle
     */
  ...
  /**
    * 读取或者重新计算对应 Partition 的数据
    *
    * 至少会使用磁盘、堆内内存、堆外内存三者之一，
    * 因此可以调用getOrCompute方法从这些存储中尝试获取计算结果 <br>
    *
    * 与 spark 1.3 版本区别很大，这里在得到数据之后并不会持久化
    *
  ...
  private[spark] def getOrCompute(partition: Partition, context: TaskContext): Iterator[T] = {
    ...
    /*
        // This method is called on executors, so we need call SparkEnv.get instead of sc.env.
         getOrElseUpdate : 用于获取 Block。如果Block存在,则获取此Block并返回 BlockResult,
    ...
    SparkEnv.get.blockManager.getOrElseUpdate(blockId, storageLevel, elementClassTag, () => {
      ...
      computeOrReadCheckpoint(partition, context)
        ...
        /*
            Left(blockResult) 与 Right(iter) 区别：
            是否启用度量信息
         */
  ...
  /**
    *  比较重要！！！是否对所有标记了需要保存检查点的祖先保存检查点
    *
    * 默认情况下,一旦找到第一个这样的RDD，我们就停止，这是一个允许我们编写的优化
    * 数据更少，但并非对所有工作负载都安全。例如，在流媒体中，我们可以对两者进行检查
    * 每个批处理中的一个RDD和它的父类，在这种情况下，父类可能永远不会被检查点
    * 它的血统从未被截断，最终导致了OOMs的产生(SPARK-6847)。
    */
  ...
  /**
    *  是否已经调用了 doCheckpoint，阻止设置多次检查点
    */
