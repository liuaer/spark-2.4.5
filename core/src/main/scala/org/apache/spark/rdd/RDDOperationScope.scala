...
/**
  *
  * 在相应的代码块中实例化的所有RDDs都将存储指向该对象的指针。
  * 示例包括但不限于现有的RDD操作，如textFile、
  * reduceByKey, treeAggregate。
  * 操作范围可以嵌套在其他范围中。例如，一个SQL查询可以封装
  * 与它在底层使用的公共RDD api相关联的范围。
  * 操作范围与阶段或作业之间没有特定的关系。
  * 范围可以存在于一个阶段(例如map)中，也可以跨越多个作业(例如take)。
  * <br><br>
