...
/**
  *
  * DAGScheduler实现了面向DAG的高层次调度，即将DAG中的各个RDD划分到不同 的Stage。
  * DAGScheduler可以通过计算将DAG中的一系列RDD划分到不同的Stage，然后构建这些Stage之间的父子关系，
  * 最后将每个Stage按照Partition切分为多个Task,并以 Task集合（即TaskSet)的形式提交给
  * 底层的TaskScheduler。所有的组件都通过向DAGScheduler投递DAGSchedulerEvent来使用
  * DAGScheduler。 DAGScheduler内部的DAGSchedulerEventProcessLoop 将处理这些
  * DAGSchedulerEvent,并调用DAGScheduler的不同方法。JobListener用于对作业中每个
  * Task执行成功或失败进行监听，Job Waiter实现了JobListener并最终确定作业的成功与失败。
...
private[spark] class DAGScheduler(
  ...
  /**
    * stageIdToStage  ------ HashMap[stageId, Stage]
    */
  private[scheduler] val stageIdToStage = new HashMap[Int, Stage]
  /**
    * shuffleIdToMapStage ------ HashMap[ShuffleId, ShuffleMapStage]
    *
  ...
  /**
    * 清空 cacheLocs 中缓存的各个RDD的所有分区位置信息
    */
  ...
  private def createResultStage(
    ...
    //  BarrierStage 先关，暂时略过
    checkBarrierStageWithDynamicAllocation(rdd)
    checkBarrierStageWithNumSlots(rdd)
    checkBarrierStageWithRDDChainPattern(rdd, partitions.toSet.size)
    // 获取所有的父 Stage，主要是宽依赖对应的Stage
    val parents = getOrCreateParentStages(rdd, jobId)
    val id = nextStageId.getAndIncrement()
  ...
  def runJob[T, U](
    ...
    // submitJob的提交是异步的，所以立即返回JobWaiter
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      /**
        ...
        *  Spark2.4.0屏障调度器
        *
        *  在Spark-2.4.0中社区提出一种新的Spark调度执行方式，名为Barrier Execution Mode，
        *  旨在通过Spark去调度分布式ML/DL(机器学习/深度学习)训练作业，这种训练作业一般是通过其他框架实现，
        *  例如MPI、TensorFlow等。由于Spark本身的计算框架是遵循MapReduce架构的，所以在调度执行时，
        *  每个Task都是独立执行的。然而，MPI这类分布式计算作业运行时需要所有Task一起执行，Task与Task之间
        *  需要相互通信。为了将诸如MPI分布式机器学习作业很好地与Spark结合，Spark社区提出一种新的BarrierScheduler，
        *  可保证所有的Task全部调起(当然要保证资源足够)。
        */
