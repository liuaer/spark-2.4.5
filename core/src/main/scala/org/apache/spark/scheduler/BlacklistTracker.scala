...
/**
  *
  * BlacklistTracker用于跟踪有问题的executors和 nodes。它支持对整个 application中的executors和nodes建立黑名单
  * (with a periodic expiry)TaskSetManagers add additional blacklisting of executors and nodes
  * for individual tasks and stages which works in concert with the blacklisting here.
  *<br><br>
  *
  * 跟踪器需要处理各种 workloads，例如:
  *
  * bad user code—— 这可能会导致许多任务失败，but that should not count against【不利于】individual executors
  *
  * many small stages ——这可以防止一个a bad executor 在一个 stage 中有许多失败，但仍然有许多失败发生在整个应用程序
  *
  * “脆弱的”执行者 —— 他们不会每项任务都失败，但仍然有足够的错误值得加入黑名单
  *
  *
...
private[scheduler] class BlacklistTracker (
  ...
  /**
    *
    *  取消已列入黑名单的executors and nodes ，时间至少是 BLACKLIST_TIMEOUT_MILLIS
    *  加入黑名单只是在一个时间范围内，到期就解除
    *
