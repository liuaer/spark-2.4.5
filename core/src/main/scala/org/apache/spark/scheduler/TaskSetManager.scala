...
/**
  * TaskSetManager也实现了 Schedulable特质，并参与到调度池的调度中。TaskSetManager
  * 对TaskSet进行管理，包括任务推断、Task本地性，并对Task进行资源分配。TaskSchedulerImpl
  * 依赖于TaskSetManager,本节将对TaskSetManager的实现进行分析。
  *
  *
...
private[spark] class TaskSetManager(
  ...
  /**
    *  Map[task所属分区Id , tasks中对应的索引]
    */
  private[scheduler] val partitionToIndex = tasks.zipWithIndex
    .map { case (t, idx) => t.partitionId -> idx }.toMap
  val numTasks = tasks.length
  /**
    * 索引位置与 taskSet 的索引一一对应<br>
    * 对每个Task的复制运行数进行记录的数组。copiesRunning按照索引与tasks数组的
    * 同一索引位置的Task相对应，记录对应Task的复制运行数量
    */
  ...
  /**
    * 索引位置与 taskSet 的索引一一对应<br>
    * 对于每个task，跟踪task的copy是否成功。任务有时候由于获取数据失败而被标记为成功，它不应该
    * 重新运行，因为丢失的map数据先被生成。
    */
  ...
  /**
    * 当任务被其他尝试任务终止时，将任务的tid添加到这个HashSet中。
    * 这发生在我们  `spark.speculation` = true 。被别人杀死的任务当executor丢失时不应重新提交。
    */
  private val killedByOtherAttempt = new HashSet[Long]
  /**
    * taskAttempts----- Array[List[TaskInfo]]<br>
    *索引位置与 taskSet 的索引一一对应<br>
    * 记录每个Task的所有执行尝试信息 List[TaskInfo]
    */
  ...
  /**
    ...
    * 就处于“僵尸”状态。例如，每个Task至少有一次尝试成功，或者TaskSet被舍弃了，TaskSetManager将会
    * 进入“僵尸”状态。 直到所有的Task都运行成功为止，TaskSetManager将一直保持在“僵尸”状态。
  ...
  /**
    ...
    *
    * 每个Executor上待处理的Task的集合。这些收藏品实际上是类的末尾添加新任务的堆栈ArrayBuffer并从末端移除。
    * 这使得检测速度更快重复失败的任务，因为每当一个任务失败时，它就被放置回到堆栈的顶部。这些集合
    * 可能包含重复项有两个原因:
    * (1):任务只是被懒懒地删除;当启动一个任务时，它将保持不变在所有待定列表中，除了启动它的那个。
    * (2):任务可能会多次重新添加到这些列表中的失败。重复项在dequeueTaskFromList中处理，
    * 这确保了任务在启动之前尚未开始运行。
    *
    */
  ...
  /**
    * 与 myLocalityLevels 中的每个本地性级别相对应，表示本地性级别的等待时间
    */
  ...
  /**
    *  当前的本地性级别在 myLocalityLevels 中的索引
    */
  ...
  /**
    ...
    */
  private var lastLaunchTime = clock.getTimeMillis()
  ...
  /** addPendingTask方法用于将待处理Task的索引按照Task的偏好位置，
    * 添加到 pendingTasksForExecutor、pendingTasksForHost、pendingTasksForRack、
    * pending-TasksWithNoPrefs、allPendingTasks 等缓存中。*/
  ...
  /**
    * 返回给定 executor 的推测性任务(如果有)。任务不应该有在此主机上运行的尝试，以防该主机运行缓慢。
    * 此外，任务应该满足给定的位置约束。
    *
  ...
  {
    // Remove finished tasks from set ，保留未完成的任务
    speculatableTasks.retain(index => !successful(index))
    // 对于speculatableTasks中的所有未在指定的Host上尝试运行，且指定的Host和Executor
    // 不在黑名单
  ...
  {
    /*
       处于“僵尸”状态并且分配Task的Host和Executor在黑名单中时直接返回None,
     */
    ...
    if (!isZombie && !offerBlacklisted) {
      ...
      dequeueTask(execId, host, allowedLocality).map { case ((index, taskLocality, speculative)) =>
        ...
        // 如果序列化Task的大小超过了100KB，且emittedTaskSizeWaming仍然为false,
        // 则将emittedTaskSizeWarning设置为true并且打印警告日志
        ...
        // 添加到runningTasksSet，同时增加父调度池中的正在运行的记录数
  ...
  private def getAllowedLocalityLevel(curTime: Long): TaskLocality.TaskLocality = {
    ...
    // moreTasksToRunIn 会调用 tasksNeedToBeScheduledFrom
    /**
      *    用于判断给定的待处理Task数组中是否有需要调度的Task。
      *    判断一个Task是否需要调度的条件是：Task没有被复制运行且Task还未执行成功。
      */
    ...
    // moreTasksToRunIn、tasksNeedToBeScheduledFrom 与 2.1 版的只是放置的位置不同
    //##########################正文###################################
  ...
  /**
    *
    *  totalResultSize > maxResultSize , 总共的结果大小是否大于限制 1G
    *
   * Check whether has enough quota[配额] to fetch the result with `size` bytes
   */
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    // 将tasklnfos中缓存的Tasklnfo标记为已经完成
    info.markFinished(TaskState.FINISHED, clock.getTimeMillis())
    if (speculationEnabled) {
      successfulTaskDurations.insert(info.duration)
    }
    // 将Task的ID从正在运行的Task集合中移除，并且减少正在运行的Task的数量
    removeRunningTask(tid)

    // Kill any other attempts for the same task (since those are unnecessary now that one
    // attempt completed successfully).
    //  杀死此Task正在运行的Task尝试, 一个 task 可能有多个尝试
    ...
    if (!successful(index)) {
      ...
      // 如果成功运行的Task数量与TaskSet中的Task数量相同，说明此TaskSet中的所有 Task都调度运行成功，于是将isZombie设置为true
      if (tasksSuccessful == numTasks) {
        isZombie = true
      }
    } else {
    ...
    // 这个阶段可能有多个tasksets ,我们让它们都知道这个分区已经完成。这可能会导致一些tasksets完成。
    ...
    // TaskSet 可能已经完成时候的一些清理工作
    maybeFinishTaskSet()
  }

  /**
    * 标记该分区的所有的 Task 运行成功，没有方法调用它，且为private可能正在开发中
    * @param partitionId
    * @param taskInfo
    */
  ...
  override def executorLost(execId: String, host: String, reason: ExecutorLossReason) {
    ...
    /*
       如果TaslcSetManager管理的TaskSet中的Task为ShuffleMapTask,并且应用没有提供外部的Shuffle服务
       且不是僵尸状态
       那么对tasklnfos中的所有在指定Executor上执行成功的Task执行以下操作
     */
    if (tasks(0).isInstanceOf[ShuffleMapTask] && !env.blockManager.externalShuffleServiceEnabled
        && !isZombie) {
      // 遍历 execId 中运行的 task信息
      for ((tid, info) <- taskInfos if info.executorId == execId) {
        ...
        if (successful(index) && !info.running && !killedByOtherAttempt.contains(tid)) {
          ...
          /*
                Resubmitted :
               * A `org.apache.spark.scheduler.ShuffleMapTask` that completed successfully earlier,
               * but we lost the executor before the stage completed. This means Spark needs to
               * reschedule the task  to be re-executed on a different executor.

               调用 DAGScheduler的taskEnded方法向DAGSchedulerEventProcessLoop发送Completion Event。
               此时的 CompletionEvent 将携带 Resubmitted,避免 CompletionEvent的关心者在Stage的所有Task
               都完成后认为当前的Stage也执行完成，而是让它们知道此Task将被重新提交。
               注意 是 Completion Event 而不是失败事件
           */
    ...
    /*
        对tasklnfos中的所有在身份标识为execld的Executor上正在运行的Task执行以下操作
     */
    for ((tid, info) <- taskInfos if info.running && info.executorId == execId) {
      // 从ExecutorLossReason ：reason 中获取Executor丢失的具体原因是否是由应用程序引起的
  ...
  /**
    *检查当前TaskSetManager是否有推断执行的任务，由TaskScheduler周期性执行<br>
    *
  ...
  override def checkSpeculatableTasks(minTimeToSpeculation: Int): Boolean = {
    ...
    // 僵尸状态、任务数=1、barrier stage >>> 没有推断执行的任务
    ...
    if (tasksSuccessful >= minFinishedForSpeculation && tasksSuccessful > 0) {
      ...
      // threshold 推断执行的最小时间 = Max(1.5* 执行时间的中间值，指定的最小推断时间)
      ...
      for (tid <- runningTasksSet) {
        ...
        if (!successful(index) && copiesRunning(index) == 1 && info.timeRunning(time) > threshold &&
          ...
          speculatableTasks += index
          // 2.1 版本没有
          // 给 eventProcessLoop 投递 推断任务提交的task
  ...
  /**
    *
    * 计算有效的本地性级别，这样就可以按照从高到低顺序分配允许的Executor
    *
  ...
  private def computeValidLocalityLevels(): Array[TaskLocality.TaskLocality] = {
    ...
    if (!pendingTasksForExecutor.isEmpty &&
      /*
         sched === TaskSchedulerlmpl

         def isExecutorAlive(execId: String): Boolean = synchronized {
              executorIdToRunningTaskIds.contains(execId)
         }

         pendingTasksForExecutor中的Executorld有存在于TaskSchedulerlmpl的
         executorldToRunningTasklds中的
       */
