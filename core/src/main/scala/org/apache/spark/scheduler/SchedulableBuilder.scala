...
/**
  *
  * schedulableQueue[TaskSetManager,TaskSetManager,TaskSetManager]
  */
...
private[spark] class FairSchedulableBuilder(val rootPool: Pool, conf: SparkConf)
  ...
  // 从用户指定的 spark.scheduler.allocation.file 来读取调度分配文件，默认从 DEFAULT_SCHEDULER_FILE 读取
  ...
  /**
    * 优先加载 spark.scheduler.allocation.file 中的文件，若找不到，则加载 fairscheduler.xml
    */
  override def buildPools() {
