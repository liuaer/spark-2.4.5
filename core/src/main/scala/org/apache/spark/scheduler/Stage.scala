...
private[scheduler] abstract class Stage(
    val id: Int, //stage ID
    val rdd: RDD[_], //当前 stage包含的RDD,一个stage只会计算一个RDD
    val numTasks: Int,
  ...
  /**
    *  Stage最近一次尝试的信息<br>
    *
    *  指向最近一次尝试的[[StageInfo]]对象的指针。这需要初始化在这里，在实际创建任何尝试之前，
    *  因为DAGScheduler使用它告诉sparklistener一个任务何时开始(在任何阶段尝试之前发生)创建了)。
    *  <br><br>
    *
  ...
  /**
    * 清空 failedAttemptIds(发生过FetchFailure的Stage尝试的身份标识的集合)
    */
