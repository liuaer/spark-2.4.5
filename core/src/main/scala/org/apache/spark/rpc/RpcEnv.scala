...
/*
 RpcEnv是Spark 2.x.x版本中新出现的组件，那么它是不是给Spark带来了新的能力呢？对于有些组件是这样的，
 但是RpcEnv组件肩负着另一项历史使命——替代Spark 2.x.x以前版本中采用的Akka。Spark移除了对Akka的使用，
 这令我感到困惑，“Akka是一款很不错的分布式消息系统，为什么要替代？正如3.2节所说的——可以让用户使用任何
 版本的Akka来编程”。Akka具有分布式集群下的消息发送、远端同步调用、远端异步调用、路由、持久化、监管、
 At LeastOnce Delivery（至少投递一次）等能力，一旦Akka被替代，那就意味着RpcEnv必须也能支持这些机制。

 抽象类RpcEnv只有一个实现子类 NettyRpcEnv
 */
/**
