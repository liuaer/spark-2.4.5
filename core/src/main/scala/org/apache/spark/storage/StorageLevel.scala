...
class StorageLevel private(
  ...
  /**
    * toInt方法实际是把 StorageLevel的_ useDisk、 useMemory、useOffHeap、 deserialized
    * 这四个属性设置到四位数字的各个状态位。例如,
    * 1000表示存储级别为允许写入磁盘;
    * 1100表示存储级别为允许写入磁盘和堆内存;
    * 1111示存储级别为允许写入磁盘、堆内存及堆外内存,并且需要反序列化。
    */
  def toInt: Int = {
    var ret = 0
    if (_useDisk) {
...
/**
  *
  * 规律：
  * 1、只能带Memory的才可以不序列化即 _deserialized = true, 原因：对象只能存在于内存中
  * 2、 带有SER: 会使用序列化
  * 3、_deserialized = true >>>>> 不序列化
  * 4、 MEMORY_ONLY : 不序列化对象
  * 5、 MEMORY_ONLY_SER 序列化对象
