...
/**
  ...
  * 的内存、磁盘及堆外内存中 Block的管理。存储体系从狭义上来说指的就是 BlockManager,
  * 从广义上来说,则包括整个Spak集群中的各个 BlockManager、 BlockInfoManager、DiskBlockManager、
  * DiskStore、 Memory Manager、 MemoryStore、对集群中的所有 BlockManager
  * 进行管理的 BlockManagerMaster及各个节点上对外提供 Block上传与下载服务的 BlockTransferService
  * <br><br>
  *
  * 每个 Driver或 Executor在创建自身的 SparkEnv时都会创建BlockManagero BlockManager只有在其
  *  initialize方法被调用后才能发挥作用
  * <br><br>
  **管理器运行在每个节点(驱动程序和执行器)，它提供了放置和的接口*从本地和远程获取块到不同的存储区(内存、磁盘和堆外)。
...
//  用于进行远程block数据传输的BlockTransferService,即5.7节介绍的 NettyBlockTransferService
    blockTransferService.init(this)
//  初始化 Shuffle客户端.有关shuffleclient的内容将在6.9节详细介绍。
    shuffleClient.init(appId)
//    设置 Block的复制策略。可以通过 spark.storage.replication.policy属性来设置
//    Block的复制策略,默认为RandomBlockReplicationPolicy。这里使用了工具类Utils的
//    classForName方法加载Clas,其具体实现可以参阅附录A。
...
//     使用到了哪些东西？
//     executorId（每个BlockManager都关联一个Executor，BlockTransferService的hostname
//     以及BlockTransferService的port）
//     所以，从BlockManagerId的初始化即可看出，一个BlockManager是通过一个节点上的Executor来唯一标识的
    val id =
      BlockManagerId(executorId, blockTransferService.hostName, blockTransferService.port, None)

//     使用BlockManagerMaster，进行BlockManager的注册
//     返回 BlockManagerId
//     slaveActor  >>> slaveEndpoint
...
//    生成 shuffleServerId。当启用了外部Shuffle服务时,将新建一个BlockManagerId作
//    为 shuffleServerId ,否则是 BlockManager自身的 BlockManagerId
...
//    当启用了外部 Shuffle服务,并且当前 BlockManager所在节点不是 Driver时,需要
//    注册外部的 Shuffle服务。
...
//      如果需要将 Block的 Blockstatus汇报给 BlockManagerMaster,则调用 tryToReportBlockStatus
//      方法,向 BlockManagerMaster汇报此Block的状态信息。
  ...
  /**
    * 此方法用于向 BlockManagerMaster重新注册 BlockManager,并向 BlockManager
    * Master报告所有的Bock信息。<br><br>
  ...
  /**
    * 获取一个 BlockManagerManagedBuffer 包装的data
  ...
  override def getBlockData(blockId: BlockId): ManagedBuffer = {
    if (blockId.isShuffle) {
      ...
      getLocalBytes(blockId) match {
        case Some(blockData) =>
          /*
             BlockManagerManagedBuffer 带有引用计数的 blockData
           */
  ...
  /**
    * 除了从 BlockInfoManager的 entries缓存中获取 BlockEd
    * 外,还需要从 DiskBlockManager中获取。这是因为 DiskBlockManager中可能存在 BlockInfo
    * Manager不知道的 Block
    * <br><br>
    *
  ...
  /**
    * <p>
    * 向 BlockManagerMaster汇报 Block的状态信息是通过调
    * 用 BlockManagerMaster的 updateBlockInfo方法完成的。
    *  BlockManagerMaster的 updateBlockInfo方法将向 BlockManagerMasterEndpoint发送
    *  UpdateBlockInfo消息。
    *   <p/>
    *
  ...
  /**
    ...
    * <p/>
    * <p>
    * 使用给定的ID返回块的更新存储状态。更具体地说，如果
    * 块从内存中删除并且可能添加到磁盘中，
    * 返回新的存储级别以及更新的内存和磁盘大小。
    * <p/>
  ...
  def getLocalBytes(blockId: BlockId): Option[BlockData] = {
    ...
    /*
      shuffleBlockResolver实现类是 IndexShuffleBlockResolver

      IndexShuffleBlockResolver：
      创建并维护shuffle块在逻辑块和物理文件位置之间的映射。
      来自同一映射任务的洗牌块的数据存储在一个统一的数据文件中。
      数据文件中数据块的偏移量存储在一个单独的索引文件中。

      shuffle数据比较特殊，需要从多个节点去获取
     */
    if (blockId.isShuffle) {
      ...
      // 不是shuffle block 直接按 blockId 去读取
  ...
  /**
    <p>
     以序列化字节的形式从本地块管理器获取块。必须在持有块上的读锁时调用。异常时释放读锁;在成功返回时保持读锁。
    <p/>
   <p>
      Get block from the local block manager as serialized bytes.
      Must be called while holding a read lock on the block.
      Releases the read lock upon exception;
      keeps the read lock upon successful return.
    <p/>
   */
  private def doGetLocalBytes(blockId: BlockId, info: BlockInfo): BlockData = {
    val level = info.level
    logDebug(s"Level for block $blockId is $level")
    //按顺序，尝试从内存中读取已序列化的字节，然后从磁盘读取，然后再返回到在内存中序列化对象，
    //最后，如果块不存在，则抛出异常。

    //deserialized 反序列化
    if (level.deserialized) {
      //试图避免昂贵的序列化，从磁盘读取一个预先序列化的副本
...
//        注意:我们故意不把块放回内存中。因为这个分支
//        处理反序列化块，此块只能作为对象缓存在内存中，不能
//        序列化的字节。因为调用方只请求字节，所以这样做没有意义
//        缓存块的反序列化对象，因为缓存可能没有回报。
  ...
  private def sortLocations(locations: Seq[BlockManagerId]): Seq[BlockManagerId] = {
    ...
    // preferredLocs 为满足条件的 otherLocs不满足条件的
    // 条件是 blockManagerId.host == loc.host
    val (preferredLocs, otherLocs) = locs.partition { loc => blockManagerId.host == loc.host }
    // topologyInfo block 的拓扑信息，代表机架之内的
    blockManagerId.topologyInfo match {
        // 理解 topologyInfo
      case None => preferredLocs ++ otherLocs
        // 将 otherLocs 分成 同一个 Rack 和不同 Rack
      case Some(_) =>
  ...
  def getRemoteBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    ...
    // 由于所有的远程块都注册在driver中,从driver端获取位置信息
    // it is not necessary to ask
    // all the slave executors to get block status.
    // locationsAndStatus   >>>>>  BlockLocationsAndStatus
    ...
    // 获取排序后的 BlockManagerId 序列
    val locations = sortLocations(blockLocations)
    val maxFetchFailures = locations.size
    var locationIterator = locations.iterator

    // 遍历每一个 BlockManagerId
    while (locationIterator.hasNext) {
      val loc = locationIterator.next()
      logDebug(s"Getting remote block $blockId from $loc")
      val data = try {
        // 使用BlockTransferService，进行，异步的远程网络获取，将block数据传输回来
        // 连接的时候，使用的BlockManager的唯一标识，就是host、port、executorId
        blockTransferService.fetchBlockSync(
          loc.host, loc.port, loc.executorId, blockId.toString, tempFileManager)
      } catch {
        /*
          如果调用 fetchBlock Sync方法时发生了异常,则增加下载失败次数( runningFailureCount)
          和下载失败总数( totalFailureCount)。当 totalFailure Count大于等于maxFetchFailures时,
          说明已经作了最大努力。当runningFailure Count大于等于maxFailuresBefore
          LocationRefresh时,则会重新调用relOcations方法刷新 Block所在的所有位置信息,并
          将 runningFailureCount清零。
         */
        case NonFatal(e) =>
...
//            放弃尝试任何locations。
//            要么是我们尝试了所有的原始位置，
//            要么是我们从主节点刷新了位置列表，但是仍然在尝试了刷新列表中的位置之后失败了。
...
//         如果有大量的executors ，那么定位列表可以包含一个大量过时的条目,这些条目可能导致大量的重试，
//        花费大量的时间。去掉这些陈旧的条目在一定数量的获取失败之后，我们将刷新块位置
          ...
          //这个位置失败了，所以我们通过返回null来重试从另一个位置获取数据
          null
      }
  ...
  /**
    * 用于获取 Block。如果Block存在,则获取此Block并返回 BlockResult,
    * 否则调用 makelterator方法计算Block,并持久化后返回 BlockResult或 Iterator
    *
  ...
  private def doPutBytes[T](
    ...
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
      ...
      if (level.useMemory) {
        ...
        //把它放在内存中，即使它也把useDisk设置为true;
        //如果内存无法保存它，我们将把它放到磁盘上
      ...
      if (blockWasSuccessfullyStored) {
        ...
        info.size = size
        // 调用reportBlockStatus()方法，将新写入的block数据发送到BlockManagerMaster
        // 以便于进行block元数据的同步和维护
  ...
  private def doPut[T](
    ...
    val result: Option[T] = try {
      ...
      /*
         如果写人成功,则在需要保持读锁的情况下将写锁降级为读锁,在不需要保持读锁
         的情况下,释放所有锁。 keepReadLock 标识是否需要保持读锁
       */
      if (res.isEmpty) {
      ...
      if (exceptionWasThrown) {
        ...
        /*
          如果抛出了异常，那么“putBody”中的代码可能已经通知了master关于这个块的可用性，
          所以我们需要发送一个更新要删除此块位置。
         */
        ...
        /*
        “putBody”代码可能还向TaskMetrics添加了一个新的块状态，所以我们需要这样做通过用空块状态覆盖它来取消它。
        我们只在finally块是通过一个异常进入的，因为无条件地这样做会导致我们为每个未能缓存的块发送空块状态内存不足
        (与未捕获的异常不同，这是预期的失败)。
         */
  ...
  /**
    ...
    *
    *
    * 尝试缓存从磁盘读取到内存存储的数据，以提高速度随后的读取。此方法要求调用方持有块上的读锁。
    *
    * <br><br>
    ...
    if (level.useMemory) {
      ...
      blockInfo.synchronized {
        if (memoryStore.contains(blockId)) {
          ...
          // 此方法将Blockld对应的Block (已经转换为Iterator)写人内存
  ...
  /**
    * 从内存中删除一个块，如果允许写磁盘，可以将它放到磁盘上。主要用于内存到到限制时需要释放空间<br>
