...
/**
  *
  * ShuffleBlockFetcherIterator 是用于获取多个Block的迭代器。如果Block在本地， 那么
  * 从本地的 BlockManager获取； 如果Block在远端， 那么通过ShuffleClient请求远端节点上
  * 的BlockTransferService获取。<br><br>
  *
  *
...
final class ShuffleBlockFetcherIterator(
    context: TaskContext,
    shuffleClient: ShuffleClient, // 此 ShuffleClient 从远端下载 block
    blockManager: BlockManager,
    /*
       将要获取的Block与所在地址的关系， 类型为 Iterator[(BlockManagerId, Seq[(BlockId, Long)])]
       从此属性可以看出， 每个 BlockManager 中包含一到多个任务需要的Block。
     */
    blocksByAddress: Iterator[(BlockManagerId, Seq[(BlockId, Long)])],
    // A function to wrap the returned input stream
    streamWrapper: (BlockId, InputStream) => InputStream,
    /*
        单次航班(即一批请求) 请求的最大字节数。这批请求的字节总数
        不能超过 maxBytesInFlight， 而且每个请求的字节数不能超过 maxBytesInFlight 的1/5。
        可以通过参数 spark.reducer.maxBytesInFlight 来控制大小(默认为48MB) 。为什么每
        个请求的字节数不能超过 maxBytesInFlight 的1/5?这样做是为了提高请求的并发
        度，保证至少向5个不同的节点发送请求获取数据，最大限度地利用各节点的资源。
     */
    maxBytesInFlight: Long,
    /*
        单次航班的最多请求数。此参数可以通过 spark.reducer.maxBytesInFlight 属性配置，
        默认为 Integer.MAX_VALUE
     */
    maxReqsInFlight: Int,
    // 每一个 host:port 能够获取的 shuffleblocks 最大数量
    maxBlocksInFlightPerAddress: Int,
    // 一个 request 能够 shuffle到内存的最大数据量
  ...
  /**
    *
    * 获取请求的队列，这些请求在第一次离开队列时不能被发出。当这些当满足 fetch约束时，将再次尝试请求。
    *
  ...
  private[this] def sendRequest(req: FetchRequest) {
    ...
    // 请求的内存量 > 一个 request 能够 shuffle到内存的最大数据量
    /*
        当从远程获取的数据量能够放到内存时，直接放到内存 results 中
        当数据太大 > maxReqSizeShuffleToMem 时，需要下载到磁盘
     */
  ...
  /**
    * 用于划分哪些Block从本地获取，哪些需要远程拉取，是获取中间计算结果的关键。
    * 远程获取的块组装一起后再发送，且不能超过最大限制
    */
  private[this] def splitLocalRemoteBlocks(): ArrayBuffer[FetchRequest] = {
    // Make remote requests at most maxBytesInFlight / 5 in length; the reason to keep them
    // smaller than maxBytesInFlight is to allow multiple, parallel fetches from up to 5
    // nodes, rather than blocking on reading output from one node.、
    // 每个远程请求的最大尺寸 maxBytesInFlight / 5
    ...
    for ((address, blockInfos) <- blocksByAddress) {
      ...
      if (address.executorId == blockManager.blockManagerId.executorId) {
        ...
        /*
           当前累加到 curBlocks 中的所有Block的大小之和，用于保证每个远
           程请求的尺寸不超过 targetRequestSize 的限制。
         */
        var curRequestSize = 0L
        /*
            curBlocks : 远程获取的累加缓存，用于保证每个远程请求的尺寸不超过target Request Size的限制。
            为什么要累加缓存?如果向一个机器节点频繁地请求字节数很小的Block，
            那么势必造成网络拥塞并增加节点负担。将多个小数据量的请求合并为一个大的请求将避免这些问题，
            提高系统性能。
         */
        ...
        while (iterator.hasNext) {
          ...
          if (size < 0) {
            ...
            // 远程获取的 block 的数量
            numBlocksToFetch += 1
            // 累加的远程请求大小，当前累加到 curBlocks 中的所有Block的大小之和
            curRequestSize += size
          }
          // 累加的远程请求大小或 当前累加到 curBlocks 中的所有Block的大小之和 > 每个远程请求的最大尺寸
          //  远程获取的block个数 > 每一个 host:port 能够获取的 shuffleblocks 最大数量
          //  >= 的理解，这里是刚好达到最大限制时候加入 将所有的请求包装成 FetchRequest, 添加到 remoteRequests
  ...
  private[this] def initialize(): Unit = {
    ...
    // 给 Task ContextImpl添加任务完成的监听器,以便于任务执行完成后调用 cleanup方法进行一些清理工作
    context.addTaskCompletionListener[Unit](_ => cleanup())

    // 划分从本地读取和需要远程读取的 Block的请求
    val remoteRequests = splitLocalRemoteBlocks()
    // 随机排序后存人 fetchRequest
  ...
  private def fetchUpToMaxBytes(): Unit = {
    ...
    /*
       疑问 当 isRemoteAddressMaxedOut 不满足条件时，即单个 host:port 发送的请求超过限制，
       但 deferredFetchRequests 的请求没有发送完怎么办？
