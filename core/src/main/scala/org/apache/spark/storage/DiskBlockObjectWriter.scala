...
/**
  *
  *
  * 我们知道, BlockManager的 getDisk Writer方法用于创建 DiskBlockObjectWritero Disk
  * BlockObject Writer将在 Shuffle阶段将map任务的输出写入磁盘,这样 reduce任务就能够
  * 从磁盘中获取map任务的中间输出了。 DiskBlockObject Writer是通过 BlockManager
  * 获取或创建的 <br>
  * DiskBlockObject Writer用于将JVM中的对象直接写入磁盘文件中。 DiskBlockObjectWriter
  * 允许将数据追加到现有 Block。为了提高效率, DiskBlockObject Writer保留了跨多个
  * 提交的底层文件通道。<br><br>
  *
...
private[spark] class DiskBlockObjectWriter(
    ...
    writeMetrics: ShuffleWriteMetrics,//用于对 Shuffle中间结果写入到磁盘的度量与统计
    val blockId: BlockId = null) // 块的唯一身份标识
