...
/**
  * BlockManagerMasterEndpoint接收Driver或 Executor上BlockManagerMaster发送的
  * 消息,对所有的 BlockManager统一管理<br>
  *
...
class BlockManagerMasterEndpoint(
  ...
  /**
    * blockManagerInfo  :: HashMap[executor ID , BlockManagerId]
    */
  private val blockManagerIdByExecutor = new mutable.HashMap[String, BlockManagerId]

  // Mapping from block id to the set of block managers that have the block.
  /**
    * blockLocations  :: new JHashMap[BlockId, HashSet[BlockManagerId]]
    *  一个 blockID 对应多个 block
    */
  ...
  /**
    *  接收blockManagerMater相关的消息<br>
    *
    *   blockManagerMater的职责<br>
  ...
  /**
    * 返回带有 topology information（如果有的话） BlockManagerId
  ...
  private def register(
    ...
    // 不指望BlockManagerId 中带有 topology information，自己获取
    ...
    if (!blockManagerInfo.contains(id)) {
      ...
      // 为BlockManagerId创建一根BlockManagerInfo，并往blockManagerInfo map中，
      // 保存一份BlockManagerId到BlockManagerInfo的映射
  ...
  /**
  更新BlockInfo，也就是说，每个BlockManager上，如果block发生了变化，那么都要发送updateBlockInfo请求
  来 EndPoint 这里。来进行BlockInfo的更新
    */
  private def updateBlockInfo(
    ...
    // 每一个block可能会在多个BlockManager上面，因为如果将StorageLevel设置成带着_2的这种，
    // 那么就需要将block replicate一份，放到其他BlockManager上，blockLocationsmap其实
    // 保存了blockId对应的BlockManagerId的set集合，所以，这里会更新blockLocations中的信息，
