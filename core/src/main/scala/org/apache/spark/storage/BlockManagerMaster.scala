...
private[spark]
/**
  * BlockManagerMaster的作用是对存在于 Executor或 Driver上的 BlockManager进行统管理。<br><br>
  * Executor与 Driver关于 BlockManager的交互都依赖于 BlockManagerMaster,比
  * 如 Executor需要向 Driver发送注册 BlockManager、更新 Executor上 Block的最新信息、
  * 询问所需要Block目前所在的位置及当 Executor运行结束需要将此 Executor移除等。
  * 但是Driver与 Executor却位于不同机器中,该怎么实现呢?<br><br>
  * 根据5.7节的内容,我们知道 Driver上的 BlockManagerMaster会实例化并且注册
  * BlockManagerMasterEndpoint无论是 Driver还是 Executor,它们的 BlockManagerMaster的
  * driverEndpoint属性都将持有 BlockManagerMasterEndpoint的 RpcEndpointRef。无论是 Driver
  * 还是 Executor,每个 BlockManager都拥有自己的 BlockManagerSlaveEndpoint,且 BlockManager
  * 的 slaveEndpoint属性保存着各自 BlockManagerSlaveEndpoint的 RpcEndpointref。 Block
  * ManagerMaster负责发送消息, BlockManagerMasterEndpoint负责消息的接收与处理,
  * BockManagerSlaveEndpoint则接收 BlockManagerMasterEndpoint下发的命令。图6-1中也展现这几着之间的关系
  * <br><br>
  *
  *   Block ManagerMaster的职责<br>
  *   BlockManagerMaster负责发送各种与存储体系相关的消息,这些消息的类型如下。<br>
  *
  * Remove Executor(移除 Executor)。<br>
  * RegisterBlockManager(注册 BlockManager)。<br>
  * UpdateBlockInfo(更新 Block信息)。<br>
  * Getlocations(获取 Block的位置)。<br>
  * GetLocationsMultipleBlockIds(获取多个Block的位置)。<br>
  * GetPeers(获取其他 BlockManager的 BlockManagerId)<br>
  * GetExecutor EndpointRef(获取 Executor的 EndpointRef引用)。<br>
  * RemoveBlock(移除 Block)。<br>
  * Removed(移除 Rdd block)。<br>
  * Remove Shuffle(移除 Shuffle block)。<br>
  * Remove Broadcast(移除 Broadcast block)。
  * GetMemory Status(获取指定的 Block Manager的内存状态)。<br>
  * GetStorageStatus(获取存储状态)。<br>
  * GetBlock Status(获取 Block的状态)。<br>
  * GetMatching BlockIds(获取匹配过滤条件的Block)。<br>
  */
