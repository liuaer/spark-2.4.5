...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    /**
      * SparkContext 的 _applicationId 属性和 _applicationAttemptId 属性是通过分别调用
      * TaskSchedulerlmpl 的 applicationld 方法和 appUcationAttemptld 方法获得的，而
      * TaskSchedulerImpl 的 applicationld 方法和 applicationAttemptld 方法实际又分别
      * 调用了 SchedulerBackend 的 applicationld 方法和 appUcationAttemptld 方法。
      *  SchedulerBackend中 appId = "spark-application-" + System.currentTimeMillis
      *
      */
